...
# Magnetic field data from Beck et. al. 2019 Tables 3 and 4
G_dat_Bord     = np.array([4.9, 5.2, 4.9, 4.6]) # Table 1 (B) in Fletcher+04
err_G_dat_Bord = 0.2*G_dat_Bord                 # Fletcher+04 mentions 20% error in B     
G_dat_Breg     = np.array([1.8, 2.1, 2.6, 2.7]) # re-computed in Beck+19
err_G_dat_Breg = 0.4*G_dat_Breg                 # Beck+19 mentions 30-40% error in B_reg
G_dat_Btot     = np.array([7.3, 7.5, 7.1, 6.3]) # Table 1 (B_tot) in Fletcher+04
err_G_dat_Btot = 0.2*G_dat_Btot                 # Fletcher+04 mentions 20% error in B_tot
#radial ranges
mrange_endps = np.array([6.8, 9.0, 11.3, 13.6, 15.8]) #lower limit of radial ranges where M is used, given in table 4, Beck+19
