...
# edited on 19/03/2024 to add 3rd argument (required for error_new.py)
# remove a column based on its name, choose to remove the corresponding radius also
def remove_data(raw_data, data_rem, adj_col_remove = True):
    ...
    if adj_col_remove:
        ...
        raw_data.drop(columns=data_removed,inplace=True)
    return raw_data
...
def incl_distance_correction(df, distance_new, distance_old, i_new, i_old):
    ...
    # Create a copy of the DataFrame to avoid modifying the original
        ...
        for col in result_df.columns:
            if substring in col: # assuming substrings are for different labels of r only
                try:    # if dist_multiplier is a list
                    result_df[col] = result_df[col] * dist_multiplier[iter_dist]
                except: # if dist_multiplier is a single number
                    result_df[col] = result_df[col] * dist_multiplier
                iter_dist += 1
            else: # the current column is the quantity corresponding to the radius 
                if 'vcirc' in col:
                    try:
                        result_df[col] = result_df[col] * incl_multiplier[1][iter_incl]
                    except:
                        result_df[col] = result_df[col] * incl_multiplier
                    iter_incl+=1
                else:
                    try:
                        result_df[col] = result_df[col] * incl_multiplier[0][iter_incl]
                    except:
                        result_df[col] = result_df[col] * incl_multiplier
                    iter_incl+=1
    ...
    df = find_and_multiply_column(df, 'kpc', distance_new/distance_old, [np.cos(i_new)/np.cos(i_old), np.sin(i_old)/np.sin(i_new)])
...
def molfrac_to_H2(df, remove_molfrac = True):
    
    df           = df.copy()
    molfrac_data = keep_substring_columns(df, 'molfrac')

    if molfrac_data[0].empty: # if no molfrac data for the galaxy
        return df
    else:
        HI_data     = keep_substring_columns(df, 'HI')
        sigma_H2    = HI_data[0].multiply((molfrac_data[0]/(1-molfrac_data[0])).values, axis = 0)
...
def vcirc_to_qomega(df, remove_vcirc = True):
    df         = df.copy()
    vcirc_data = keep_substring_columns(df, 'vcirc')
